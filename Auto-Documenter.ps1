<#
.SYNOPSIS
   Auto Documentor for AppCheck-NG API Client Module
.AUTHOR
   Chris Harris (chris@utopianit.co.uk)
.DESCRIPTION
   Automatically Documents the AppCheck-NG API Client Module in Markdown and HTML formats
   Uses code from various sources including ConvertFrom-MarkDown By https://gist.github.com/lidopaglia
.EXAMPLE
   Auto-Documenter.ps1
.VERSION
   20201125
...
param
(
    [string]$PathToModule='.\AppCheck-NG.psm1',
    [string]$ModuleName = 'AppCheck-NG',
    [string]$ModuleTitle = 'AppCheck-NG API Client',
    [string]$OutputFolderPath='.\Docs',
    [string]$ResultFileNamePrefix='AppCheck-NG',
    [string]$ResultFileNameSuffix = 'md',
    [string]$ResultFileNameSuffixHTML = 'html'
)
...
.DESCRIPTION
   Original By https://gist.github.com/lidopaglia
...
#>
    ...
    Process {
        ...
        Foreach ($item in $InputObject) {
            ...
            <head>
	            <meta charset="UTF-8">
	            <title>$ModuleTitle PowerShell Module</title>
	            <style>
...
function Generate-TemporaryFile
{
    ...
    "# $ModuleTitle PowerShell Module ($ModuleName) `r`n" | Out-File -FilePath $outputFile  -Force
...
# Convert to UTF-8 MD file and HTML afterwards 
$finalFileName     = [string]::Format('{0}\{1}.{2}',$OutputFolderPath,$ResultFileNamePrefix,$ResultFileNameSuffix)
$finalFileNameHTML = [string]::Format('{0}\{1}.{2}',$OutputFolderPath,$ResultFileNamePrefix,$ResultFileNameSuffixHTML)

[System.Io.File]::ReadAllText($outputFile) | Out-File -FilePath $finalFileName -Encoding utf8
Get-Content $finalFileName -Raw | ConvertFrom-Markdown | Out-File -FilePath $finalFileNameHTML -Encoding utf8
